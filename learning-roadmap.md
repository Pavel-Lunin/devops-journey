...
- Перед контрольными вопросами сначала даём достаточную теорию простым языком: определение, бытовую аналогию, пример с текущего VPS и команды для наблюдения.
- Не спрашиваем термины, которые ещё не были объяснены. Если термин нужен раньше времени, сначала объясняем его отдельно.
- Идём от простого к сложному: наблюдаем факт на сервере, называем его, объясняем смысл, только потом связываем с архитектурой.
...
1. Простое объяснение темы: что это, зачем нужно, на что похоже.
2. Мини-словарь терминов урока.
3. Разбор на твоём VPS или рабочем CI-контексте.
4. Практическое задание руками только read-only, если тема новая.
5. Совместная расшифровка вывода команд.
6. Контрольные вопросы после объяснения, а не до него.
7. Мини-конспект в `learning-log`.
8. Обновление backlog/roadmap, если появились новые выводы.

Педагогический стандарт для каждого нового термина:

- **Определение:** одно простое объяснение без перегруза.
- **Аналогия:** бытовой пример, если он помогает.
- **Пример на VPS:** конкретный объект с этого сервера.
- **Команда:** как посмотреть это самому.
- **Риск:** что можно сломать, если менять без понимания.
- **Проверка понимания:** короткий вопрос только после объяснения.
...
#### Урок 0.5. HTTP И curl

Цель: научиться проверять HTTP-сервисы и API руками, понимать request/response и использовать `curl` как базовый инструмент диагностики.

Что разбираем:

- что такое HTTP request и HTTP response;
- методы `GET`, `POST`, `HEAD`;
- status codes: `200`, `301`, `302`, `400`, `401`, `403`, `404`, `500`;
- headers и body;
- `curl -I`, `curl -v`, `curl -sS`, `curl -f`, `curl --max-time`;
- JSON request body;
- bearer token и basic auth без вывода secrets;
- чем диагностика через `curl` отличается от проверки "открывается в браузере".

Практическое задание:

- выполнить `curl -I http://127.0.0.1/` на VPS;
- выполнить `curl --max-time 5 -fsSI http://127.0.0.1:8080/`;
- разобрать первые строки ответа: status code, server, content type;
- выполнить безопасный внешний запрос к публичному endpoint, например `curl -I https://example.com`;
- составить шпаргалку из 10 команд `curl`, которые пригодятся для VPS, бота и Jenkins pipeline.

Практика на работе:

- понять, какие HTTP endpoints вызываются из Jenkins pipeline;
- научиться проверять доступность Nexus/SberOSC/Vault/API публикации без раскрытия токенов;
- разобрать, чем отличаются `401`, `403`, `404`, `5xx` для pipeline failures.

Практика на боте:

- если у бота появится `/health`, проверять его через `curl`;
- если появится API endpoint, отправлять JSON через `curl -X POST`;
- добавить в README примеры диагностики.

Критерии готовности:

- можешь объяснить разницу между `curl`, браузером и Postman;
- можешь прочитать HTTP status code и понять направление проблемы;
- умеешь выполнить request с headers без вывода secrets в терминал;
- понимаешь, почему `curl -f` полезен в CI.

#### Урок 0.6. Docker И AmneziaWG
...
#### Урок 0.7. Рабочий Mobile CI/CD Pipeline
...
#### Урок 0.8. Telegram-Бот Как Учебный Сервис
...
- HTTP diagnostics: `curl`, headers, status codes, JSON requests, timeouts, auth without leaking secrets.
...
- Сделать `curl`-шпаргалку для проверки localhost, внешних endpoints, headers, status codes и timeouts.
...
- Сделать `curl` mini-lab: 10 команд для VPS, Telegram-бота и Jenkins/Nexus/Vault диагностики без вывода secrets.
